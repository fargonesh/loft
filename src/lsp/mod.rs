...
use crate::parser::{Expr, InputStream, Parser, Stmt, TemplatePart, Type};
...
impl LoftLanguageServer {
    ...
    async fn load_exported_symbols(&self, uri: &Uri) -> Vec<SymbolInfo> {
        ...
        match parser.parse() {
            Ok(stmts) => {
                ...
                // Return only exported symbols
                symbols.into_iter().filter(|s| s.is_exported).collect()
            }
            Err(_) => Vec::new(),
        }
    }
    ...
    async fn parse_and_report_diagnostics(&self, uri: &Uri, content: &str) {
        ...
        let semantic_diagnostics =
            Self::check_semantic_errors(&stmts, &symbols, &content_string, &self.stdlib_types);
    ...
    fn associate_doc_comments(source: &str, symbols: &mut [SymbolInfo]) {
        ...
        let mut doc_map: std::collections::HashMap<String, String> =
    ...
    fn check_trait_implementations(
        ...
        for stmt in stmts {
            ...
            } = stmt
            {
                ...
                let missing_methods: Vec<String> = required_methods
                    .into_iter()
            ...
            if let Stmt::Block(inner_stmts) = stmt {
                Self::check_trait_implementations(
                    inner_stmts,
                    symbols,
                    stdlib_types,
                    diagnostics,
    ...
    fn check_semantic_errors(
        ...
        Self::check_stmt_list_with_imports(
            stmts,
            symbols,
            &mut used_variables,
            &mut used_imports,
        ...
        match expr {
            Expr::Ident(name) => {
                ...
                if !symbols.iter().any(|s| &s.name == name) {
                    ...
                    let builtin_modules = [
                        "term", "math", "time", "web", "fs", "console", "json", "encoding",
                        "random",
                    ];
        ...
        for symbol in symbols {
            if let SymbolKind::Function { .. } = &symbol.kind {
                if let Some(range) = &symbol.range {
                    if (range.start.line as usize) <= line_num
                        && (range.end.line as usize) >= line_num
                    {
                        return Some(symbol);
                    }
                }
            ...
            match stmt {
                Stmt::VarDecl {
                    ...
                    let inferred_type = if var_type.is_none() {
                        value
                            .as_ref()
                            .and_then(|v| Self::infer_type_from_expr(v, &symbols, stdlib_types))
                    } else {
                        None
                    };
                ...
                Stmt::TraitDecl { name, methods } => {
                    let method_infos: Vec<TraitMethodInfo> = methods
                        .iter()
                        .map(|m| match m {
                            crate::parser::TraitMethod::Signature {
                                name,
                                params,
                                return_type,
                            } => TraitMethodInfo {
                            ...
                            crate::parser::TraitMethod::Default {
                                name,
                                params,
                                return_type,
                                ..
            ...
            } = stmt
            {
                ...
                for method in methods {
                    ...
                    } = method
                    {
                        ...
                        // Recursively extract symbols from function body
                        if let Stmt::Block(body_stmts) = body.as_ref() {
                            method_symbols.extend(Self::extract_symbols(
                                body_stmts,
                                scope_level + 2,
                                stdlib_types,
                            ));
                ...
                } = &symbol.kind
                {
                    ...
                    // Handle user-defined structs
                        ...
                        if let Some(struct_symbol) =
                            doc_data.symbols.iter().find(|s| s.name == *type_name)
                        {
...
pub(crate) fn get_field_access_at_position(line: &str, col: usize) -> Option<(String, String)> {
    ...
    while field_start > 0
        && (chars[field_start - 1].is_alphanumeric() || chars[field_start - 1] == '_')
    {
        field_start -= 1;
    }
    ...
    while object_start > 0
        && (chars[object_start - 1].is_alphanumeric() || chars[object_start - 1] == '_')
    {
        object_start -= 1;
    }
...
impl LanguageServer for LoftLanguageServer {
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        ...
        self.parse_and_report_diagnostics(&params.text_document.uri, &content)
            .await;
    }
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        self.client
            .log_message(MessageType::LOG, format!("Hovering over word: '{}'", word))
            .await;
        ...
        if let Some((object_name, method_name)) = get_field_access_at_position(line, col) {
            ...
            let visible_symbols: Vec<&SymbolInfo> = doc_data
                .symbols
                .iter()
        ...
        self.client
            .log_message(
                MessageType::LOG,
                format!(
                    "Found {} visible symbols (out of {} total)",
                    visible_symbols.len(),
                    doc_data.symbols.len()
                ),
            )
            .await;
        ...
        let local_symbol = visible_symbols
            .iter()
        ...
        if let Some(symbol) = local_symbol {
            self.client
                .log_message(
                    MessageType::LOG,
                    format!(
                        "Found local symbol '{}' at scope level {}",
                        symbol.name, symbol.scope_level
                    ),
                )
                .await;
        ...
        if let Some(imported_symbol) = doc_data.imported_symbols.iter().find(|s| s.name == word) {
            self.client
                .log_message(
                    MessageType::LOG,
                    format!(
                        "Found imported symbol '{}' from {:?}",
                        imported_symbol.name, imported_symbol.source_uri
                    ),
                )
                .await;
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        if let Some(doc_data) = doc_data {
            ...
            if (position.line as usize) < lines.len() {
                let line = lines[position.line as usize];
                let context_text =
                    Self::get_context_before_cursor(line, position.character as usize);
                ...
                if Self::is_in_import_string(context_text) {
                    ...
                    let file_path =
                        Self::uri_to_file_path(&params.text_document_position.text_document.uri);
        ...
        if let Some(doc_data) = docs.get(&uri) {
            // Determine the scope level at the cursor position
            let cursor_scope =
                Self::get_scope_at_position(&doc_data.content, position.line as usize);
            ...
            self.client
                .log_message(
                    MessageType::LOG,
                    format!(
                        "Offering {} visible symbols for completion (out of {} total)",
                        visible_symbols.len(),
                        doc_data.symbols.len()
                    ),
                )
                ...
                let (kind, insert_text) = match &symbol.kind {
                    ...
                    SymbolKind::Function { params, .. } => {
                        ...
                        (
                            ...
                            format!("{}({})$0", symbol.name, params_snippet),
                        )
                    }
                    SymbolKind::Struct { .. } => (CompletionItemKind::STRUCT, symbol.name.clone()),
                    SymbolKind::Trait { .. } => {
                        (CompletionItemKind::INTERFACE, symbol.name.clone())
                    }
                };
            ...
            if let Some(range) = &symbol.range {
                ...
                self.client
                    .log_message(
                        MessageType::LOG,
                        format!(
                            "Found local definition for '{}' at line {}",
            ...
            if let Some(doc_data) = docs.get(&uri) {
                ...
                if let Some(symbol) = visible_symbols.iter().find(|s| s.name == word) {
                    if let Some(range) = &symbol.range {
                        ...
                        if !locations
                            .iter()
                            .any(|loc| loc.range.start == def_loc.range.start)
                        {
        ...
        if let Some(paren_pos) = before_cursor.rfind('(') {
            ...
            if let Some(symbol) = visible_symbols.iter().find(|s| s.name == func_name) {
                ...
                } = &symbol.kind
                {
                    ...
                    let parameter_info: Vec<ParameterInformation> = params
                        .iter()
                        .map(|(name, typ)| ParameterInformation {
            ...
            for (builtin_name, builtin) in &self.stdlib_types.builtins {
                if let Some(method) = builtin.methods.get(func_name) {
                    ...
                    let parameter_info: Vec<ParameterInformation> = method
                        .params
                        .iter()
            ...
            if let Some(last_word_start) =
                struct_pattern.rfind(|c: char| !c.is_alphanumeric() && c != '_')
            {
                let potential_struct = &struct_pattern[last_word_start + 1..];
                if !potential_struct.is_empty() {
                    // Check if this is a struct
                    if let Some(symbol) =
                        visible_symbols.iter().find(|s| s.name == potential_struct)
                    {
                        if let SymbolKind::Struct { fields, .. } = &symbol.kind {
                            ...
                            let label =
                                format!("{} {{ {} }}", potential_struct, field_strings.join(", "));

                            let parameter_info: Vec<ParameterInformation> = fields
                                .iter()
                                .map(|(name, typ)| ParameterInformation {
                                    label: ParameterLabel::Simple(format!("{}: {}", name, typ)),
                                    documentation: None,
    ...
    async fn formatting(&self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        ...
        self.client
            .log_message(MessageType::LOG, format!("Formatting request for {}", uri))
            .await;
        ...
        match formatter.format(&doc_data.content) {
            Ok(formatted) => {
                ...
                if formatted != doc_data.content {
                    ...
                    self.client
                        .log_message(MessageType::LOG, "No formatting changes needed".to_string())
                        .await;
                    Ok(None)
                }
            }
    ...
    async fn semantic_tokens_full(
        ...
        let keywords = &[
            "learn", "teach", "fn", "let", "mut", "const", "if", "else", "while", "for", "in",
            "return", "break", "continue", "def", "impl", "trait", "enum", "match", "async",
            "await", "lazy",
        ];
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            while col < chars.len() {
                ...
                if chars[col] == '"' || chars[col] == '\'' || chars[col] == '`' {
                    ...
                    let delta_line = if line_num as u32 == prev_line {
                        0
                    } else {
                        line_num as u32 - prev_line
                    };
                    let delta_start = if delta_line == 0 {
                        start_col as u32 - prev_start
                    } else {
                        start_col as u32
                    };
    ...
    async fn semantic_tokens_range(
        ...
        if let Some(SemanticTokensResult::Tokens(tokens)) =
            self.semantic_tokens_full(full_params).await?
        {
    ...
    async fn code_action(&self, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
        ...
        self.client
            .log_message(MessageType::LOG, format!("Code action request for {}", uri))
            .await;
        ...
        for diagnostic in &params.context.diagnostics {
            ...
            if diagnostic
                .message
                .contains("Missing implementation for methods")
            {
                    ...
                    if let Ok(info) = serde_json::from_value::<serde_json::Value>(data.clone()) {
                        if let Some(missing_methods) = info["missing_methods"].as_array() {
                            ...
                            for method_val in missing_methods {
                                ...
                                // Check user-defined traits
                                if let Some(trait_symbol) =
                                    doc_data.symbols.iter().find(|s| s.name == trait_name)
                                {
                                    if let SymbolKind::Trait { methods } = &trait_symbol.kind {
                                        if let Some(m) =
                                ...
                                if signature.is_none() {
                                    if let Some(trait_def) =
                                        self.stdlib_types.traits.get(trait_name)
                                    {
                                        if let Some(method_def) = trait_def.methods.get(method_name)
                                        {
                                            let params: Vec<(String, String)> = method_def
                                                .params
                                                .iter()
                                                .map(|p| {
                            ...
                            if let Some(end_line) =
                                Self::find_closing_brace(&doc_data.content, start_line)
                            {
                                let mut changes = HashMap::new();
                                changes.insert(
        ...
        if range.start.line < lines.len() as u32 {
            ...
            if !word.is_empty() {
                ...
                let cursor_scope =
                    Self::get_scope_at_position(&doc_data.content, range.start.line as usize);
                let visible_symbols: Vec<&SymbolInfo> = doc_data
                    .symbols
                    .iter()
                    .filter(|s| s.scope_level <= cursor_scope)
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            // Import blocks (multiple consecutive learn statements)
                ...
                if brace_stack.is_empty()
                    || brace_stack.last().map(|(_, k)| k) != Some(&FoldingRangeKind::Imports)
                {
    ...
    async fn document_link(&self, params: DocumentLinkParams) -> Result<Option<Vec<DocumentLink>>> {
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            while let Some(start_idx) = line[search_from..]
                .find("http://")
...
mod tests {
    ...
    fn load_stdlib_types() -> StdlibTypes {
        let stdlib_json = include_str!("stdlib_types.json");
        serde_json::from_str::<StdlibTypes>(stdlib_json).expect("Failed to parse stdlib_types.json")
    }
...
"#;
    ...
    fn test_doc_comment_link_processing() {
        ...
        assert_eq!(
            processed1,
            "This function uses `Vec` and `std::collections::HashMap`"
        );
        ...
        assert_eq!(
            processed2,
            "See `Option` and `Result` for error handling. Also check `mod::Item`."
        );
...
"#;
    ...
    fn test_member_access_context() {
        ...
        assert_eq!(LoftLanguageServer::get_member_access_context("myvar"), None);
...
"#;
    ...
    fn test_response_type_completions() {
        ...
        assert!(
            response_type.fields.contains_key("status"),
            "Response should have status field"
        );
        assert!(
            response_type.fields.contains_key("headers"),
            "Response should have headers field"
        );
        assert!(
            response_type.fields.contains_key("body"),
            "Response should have body field"
        );

        // Check that Response has expected methods
        assert!(
            response_type.methods.contains_key("json"),
            "Response should have json method"
        );
        assert!(
            response_type.methods.contains_key("text"),
            "Response should have text method"
        );

        // Check field types
        assert_eq!(
            response_type.fields.get("status").unwrap().field_type,
            "num"
        );
        assert_eq!(
            response_type.fields.get("headers").unwrap().field_type,
            "Headers"
        );
        assert_eq!(
            response_type.fields.get("body").unwrap().field_type,
            "Promise<Buffer>"
        );

        // Check method return types
        assert_eq!(
            response_type.methods.get("json").unwrap().return_type,
            "Promise<Object>"
        );
        assert_eq!(
            response_type.methods.get("text").unwrap().return_type,
            "Promise<str>"
        );
    }
    ...
    fn test_request_builder_type_completions() {
        ...
        assert!(
            builder_type.is_some(),
            "RequestBuilder type should be defined"
        );
        ...
        assert!(
            builder_type.methods.contains_key("method"),
            "RequestBuilder should have method method"
        );
        assert!(
            builder_type.methods.contains_key("header"),
            "RequestBuilder should have header method"
        );
        assert!(
            builder_type.methods.contains_key("body"),
            "RequestBuilder should have body method"
        );
        assert!(
            builder_type.methods.contains_key("timeout"),
            "RequestBuilder should have timeout method"
        );
        assert!(
            builder_type.methods.contains_key("followRedirects"),
            "RequestBuilder should have followRedirects method"
        );
        assert!(
            builder_type.methods.contains_key("send"),
            "RequestBuilder should have send method"
        );

        // Check that send returns Promise<Response>
        assert_eq!(
            builder_type.methods.get("send").unwrap().return_type,
            "Promise<Response>"
        );
    }
...
"#
        ...
        let completions = server
            .get_member_completions("response", &uri)
            .await
            .unwrap();

        assert!(
            completions.is_some(),
            "Should have completions for Response type"
        );
        ...
        if let Some(CompletionResponse::Array(items)) = completions {
            ...
            assert!(
                labels.contains(&"headers".to_string()),
                "Should have headers field"
            );
            assert!(
                labels.contains(&"body".to_string()),
                "Should have body field"
            );
            assert!(
                labels.contains(&"json".to_string()),
...
"#
        ...
        assert!(
            completions.is_some(),
            "Should have completions for RequestBuilder type"
        );
        ...
        if let Some(CompletionResponse::Array(items)) = completions {
            ...
            assert!(
                ...
                "Should have header method"
            );
            assert!(
                labels.contains(&"body".to_string()),
                "Should have body method"
            );
            assert!(
                labels.contains(&"timeout".to_string()),
                "Should have timeout method"
            );
            assert!(
                labels.contains(&"followRedirects".to_string()),
                "Should have followRedirects method"
            );
            assert!(
...
"#
        ...
        if let Some(CompletionResponse::Array(items)) = completions_a {
            ...
            assert_eq!(
                items.len(),
                0,
                "Promise<Response> should have no completions"
            );
        }
        ...
        if let Some(CompletionResponse::Array(items)) = completions_b {
            ...
            assert!(
                items.len() >= 5,
                "Should have at least 5 completions for Response"
            );
        } else {
