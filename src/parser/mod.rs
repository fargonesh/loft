...
impl<'a> Parser<'a> {
    ...
    fn synchronize(&mut self) {
        ...
        while let Ok(Some(token)) = self.peek() {
            ...
            if let Token::Keyword(k) = &token {
                match k.as_str() {
                    "fn" | "let" | "const" | "if" | "while" | "for" | "return" | "teach" | "learn" | "def" | "impl" | "trait" => return,
                    _ => {}
                }
            }
                    ...
                    if let Token::Ident(name) = &token {
                        ...
                        if let Some(Token::Op(op)) = self.peek()? {
                            if op == "=" {
                                ...
                                self.maybe_consume_semicolon();
                                return Ok(Stmt::Assign { name: name_clone, value });
                            }
                        }
    ...
    fn parse_type(&mut self) -> Result<Type> {
        ...
        let name = match name_token {
            ...
            None => return Err(self.tokens.croak("Expected type name but got EOF".to_string(), None)),
        };
                ...
                loop {
                    ...
                    if let Some(token) = self.peek()? {
                        if self.is_punct(&token, ",") {
                            ...
                            return Err(self.tokens.croak("Expected ',' or '>' in generic type".to_string(), None));
                        }
                    } else {
                        return Err(self.tokens.croak("Unexpected EOF in generic type".to_string(), None));
                    }
                }
    ...
    fn parse_import_statement(&mut self) -> Result<Stmt> {
        ...
        let module_str = match module_token {
            ...
            Some(token) => return Err(self.tokens.croak(format!("Expected string literal after 'learn' but got {}", token), None)),
            None => return Err(self.tokens.croak("Expected string literal after 'learn' but got EOF".to_string(), None)),
        };
        ...
        if path.is_empty() {
            return Err(self.tokens.croak("Import path cannot be empty".to_string(), None));
        }
    ...
    fn parse_function_decl(&mut self, is_async: bool, is_exported: bool) -> Result<Stmt> {
            ...
            if op == "<" {
                ...
                loop {
                    ...
                    if let Some(Token::Ident(param)) = param_token {
                        ...
                        return Err(self.tokens.croak("Expected type parameter name".to_string(), None));
                    }
                    ...
                    if let Some(token) = self.peek()? {
                        if self.is_punct(&token, ",") {
                            ...
                            return Err(self.tokens.croak("Expected ',' or '>' in type parameters".to_string(), None));
                        }
                    }
                }
            }
        ...
        while let Some(token) = self.peek()? {
            ...
            let param_name = match self.next()? {
                Some(Token::Ident(name)) => name,
                Some(token) => return Err(self.tokens.croak(format!("Expected parameter name but got {}", token), None)),
                None => return Err(self.tokens.croak("Expected parameter name but got EOF".to_string(), None)),
            };
    ...
    fn parse_struct_decl(&mut self) -> Result<Stmt> {
        ...
        let name = match name_token {
            Some(Token::Ident(name)) => name,
            Some(token) => return Err(self.tokens.croak(format!("Expected struct name but got {}", token), None)),
            None => return Err(self.tokens.croak("Expected struct name but got EOF".to_string(), None)),
        };
        ...
        while let Some(token) = self.peek()? {
            ...
            let field_name = match self.next()? {
                Some(Token::Ident(name)) => name,
                Some(token) => return Err(self.tokens.croak(format!("Expected field name but got {}", token), None)),
                None => return Err(self.tokens.croak("Expected field name but got EOF".to_string(), None)),
            };
    ...
    fn parse_enum_decl(&mut self) -> Result<Stmt> {
        ...
        let name = match name_token {
            Some(Token::Ident(name)) => name,
            Some(token) => return Err(self.tokens.croak(format!("Expected enum name but got {}", token), None)),
            None => return Err(self.tokens.croak("Expected enum name but got EOF".to_string(), None)),
        };
        ...
        while let Some(token) = self.peek()? {
            ...
            let variant_name = match self.next()? {
                Some(Token::Ident(name)) => name,
                Some(token) => return Err(self.tokens.croak(format!("Expected variant name but got {}", token), None)),
                None => return Err(self.tokens.croak("Expected variant name but got EOF".to_string(), None)),
            };
    ...
    fn parse_trait_decl(&mut self) -> Result<Stmt> {
        ...
        let name = match name_token {
            Some(Token::Ident(name)) => name,
            Some(token) => return Err(self.tokens.croak(format!("Expected trait name but got {}", token), None)),
            None => return Err(self.tokens.croak("Expected trait name but got EOF".to_string(), None)),
        };
        ...
        while let Some(token) = self.peek()? {
            ...
            let method_name = match self.next()? {
                Some(Token::Ident(name)) => name,
                Some(token) => return Err(self.tokens.croak(format!("Expected method name but got {}", token), None)),
                None => return Err(self.tokens.croak("Expected method name but got EOF".to_string(), None)),
            };
    ...
    fn parse_impl_block(&mut self) -> Result<Stmt> {
        ...
        let first_name = match self.next()? {
            Some(Token::Ident(name)) => name,
            Some(token) => return Err(self.tokens.croak(format!("Expected type or trait name but got {}", token), None)),
            None => return Err(self.tokens.croak("Expected type or trait name but got EOF".to_string(), None)),
        };
    ...
    fn parse_for_statement(&mut self) -> Result<Stmt> {
        ...
        let var = match self.next()? {
            Some(Token::Ident(name)) => name,
            _ => return Err(self.tokens.croak("Expected variable name".to_string(), None)),
        };
    ...
    fn parse_pattern_primary(&mut self) -> Result<Expr> {
        ...
        match token_opt {
            ...
            Some(token) => Err(self.tokens.croak(format!("Unexpected token in pattern: {}", token), None)),
            None => Err(self.tokens.croak("Unexpected EOF in pattern".to_string(), None)),
        }
    }
    ...
    fn parse_lambda_with_parens(&mut self) -> Result<Expr> {
        ...
        while let Some(token) = self.peek()? {
            ...
            let param_name = match self.next()? {
                Some(Token::Ident(name)) => name,
                Some(token) => {
                    return Err(self.tokens.croak(format!("Expected parameter name but got {}", token), None));
                }
                None => {
                    return Err(self.tokens.croak("Expected parameter name but got EOF".to_string(), None));
                }
            };
    ...
    fn parse_lambda_body(&mut self, params: Vec<(String, Option<Type>)>, return_type: Option<Type>) -> Result<Expr> {
        // Body
...
impl<'a> TokenStream<'a> {
    pub fn new(input: InputStream<'a>) -> Self {
        Self { input, buffer: Vec::new(), last_doc_comment: None }
    }
