...
pub fn create_math_builtin() -> BuiltinStruct {
    ...
    math.add_field(
        "PI",
        Value::Number(Decimal::from_f64_retain(std::f64::consts::PI).unwrap()),
    );
    math.add_field(
        "E",
        Value::Number(Decimal::from_f64_retain(std::f64::consts::E).unwrap()),
    );
    math.add_field(
        "TAU",
        Value::Number(Decimal::from_f64_retain(std::f64::consts::TAU).unwrap()),
    );
