...
impl From<Buffer> for Value {
    fn from(buffer: Buffer) -> Self {
        ...
        fields.insert("data".to_string(), Value::Array(
            buffer.data.into_iter().map(|b| Value::Number(Decimal::from(b))).collect()
        ));
                    ...
                    for item in data_array {
                        if let Value::Number(n) = item {
                            if let Some(byte) = n.to_u8() {
                                ...
                                return Err(RuntimeError::new("Buffer data must contain valid bytes (0-255)"));
                            }
                        } else {
...
impl From<HttpResponse> for Value {
    fn from(response: HttpResponse) -> Self {
        let mut fields = HashMap::new();
        fields.insert("status".to_string(), Value::Number(Decimal::from(response.status)));
        ...
        fields.insert("headers".to_string(), Value::Struct {
...
impl From<RequestBuilder> for Value {
    fn from(builder: RequestBuilder) -> Self {
        ...
        fields.insert("method".to_string(), Value::String(format!("{:?}", builder.method)));
        ...
        fields.insert("followRedirects".to_string(), Value::Boolean(builder.follow_redirects));
...
fn web_method(_interpreter: &mut Interpreter, this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    Err(RuntimeError::new("method() can only be called on RequestBuilder"))
}
...
fn web_header(_interpreter: &mut Interpreter, this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    if let Value::Struct { name, mut fields } = this.clone() {
        if name == "RequestBuilder" {
            ...
            let mut header_fields = HashMap::new();
            if let Some(Value::Struct { fields: existing_headers, .. }) = fields.get("headers") {
    ...
    Err(RuntimeError::new("header() can only be called on RequestBuilder"))
}
...
fn web_body(_interpreter: &mut Interpreter, this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    Err(RuntimeError::new("body() can only be called on RequestBuilder"))
}
...
fn web_timeout(_interpreter: &mut Interpreter, this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    let timeout_ms = match &args[0] {
        Value::Number(n) => {
            n.to_u64().ok_or_else(|| RuntimeError::new("timeout must be a positive number"))?
        },
    ...
    if let Value::Struct { name, mut fields } = this.clone() {
        if name == "RequestBuilder" {
            fields.insert("timeout".to_string(), Value::Number(Decimal::from(timeout_ms)));
            return Ok(Value::Struct { name, fields });
        }
    }
    
    Err(RuntimeError::new("timeout() can only be called on RequestBuilder"))
}
...
fn web_follow_redirects(_interpreter: &mut Interpreter, this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    let follow = match &args[0] {
        Value::Boolean(b) => *b,
        _ => return Err(RuntimeError::new("followRedirects() argument must be a boolean")),
    };
    ...
    Err(RuntimeError::new("followRedirects() can only be called on RequestBuilder"))
}
...
fn web_send(_interpreter: &mut Interpreter, this: &Value, _args: &[Value]) -> RuntimeResult<Value> {
    if let Value::Struct { name, fields } = this {
        if name == "RequestBuilder" {
            ...
            // Add headers
            if let Some(Value::Struct { fields: header_fields, .. }) = fields.get("headers") {
    ...
    Err(RuntimeError::new("send() can only be called on RequestBuilder"))
}
...
fn web_buffer(_interpreter: &mut Interpreter, _this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    match &args[0] {
        ...
        _ => Err(RuntimeError::new("Buffer can be created from string or array of numbers")),
    }
}
