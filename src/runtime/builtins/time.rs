...
fn time_perf_now(_this: &Value, _args: &[Value]) -> RuntimeResult<Value> {
    ...
    Ok(Value::Number(Decimal::try_from(millis).unwrap_or(Decimal::from(0))))
}
...
fn time_format(_this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    let duration_ms = match &args[0] {
        ...
        _ => return Err(RuntimeError::new("time.format() argument must be a number (milliseconds)")),
    };
...
fn time_benchmark(_this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    ...
    Ok(Value::String("Benchmark not yet fully implemented".to_string()))
}
