...
fn url_encode(_this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    if args.is_empty() {
        return Err(RuntimeError::new(
            "encoding.url_encode() requires a string argument",
        ));
    }
    ...
    let input = match &args[0] {
        ...
        _ => {
            return Err(RuntimeError::new(
                "encoding.url_encode() argument must be a string",
            ))
        }
    };
...
fn url_decode(_this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    if args.is_empty() {
        return Err(RuntimeError::new(
            "encoding.url_decode() requires a string argument",
        ));
    }
    ...
    let input = match &args[0] {
        ...
        _ => {
            return Err(RuntimeError::new(
                "encoding.url_decode() argument must be a string",
            ))
        }
    };
...
fn to_bytes(_this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    if args.is_empty() {
        return Err(RuntimeError::new(
            "encoding.to_bytes() requires a string argument",
        ));
    }
    ...
    let input = match &args[0] {
        ...
        _ => {
            return Err(RuntimeError::new(
                "encoding.to_bytes() argument must be a string",
            ))
        }
    };

    let bytes: Vec<Value> = input
        .as_bytes()
        .iter()
...
fn from_bytes(_this: &Value, args: &[Value]) -> RuntimeResult<Value> {
    if args.is_empty() {
        return Err(RuntimeError::new(
            "encoding.from_bytes() requires an array argument",
        ));
    }
    ...
    let bytes_array = match &args[0] {
        ...
        _ => {
            return Err(RuntimeError::new(
                "encoding.from_bytes() argument must be an array",
            ))
        }
    };
