...
`);
  ...
  const examples = [
    {
      ...
      code: `term.println("Hello, loft!");`
    },
    {
      name: "Factorial",
      description: "Recursive factorial function.",
      code: `fn factorial(n: num) -> num {\n  if (n <= 1) { return 1; }\n  return n * factorial(n - 1);\n}\n\nterm.println("Factorial of 5: " + factorial(5));`
    },
    {
      name: "Closures",
      description: "Using closures and lexical scope.",
      code: `let multiplier = 3;\nlet triple = (x: num) => x * multiplier;\nterm.println("Triple 7: " + triple(7));`
    },
    {
      name: "Enums & Matching",
      description: "Algebraic data types and pattern matching.",
      code: `enum Status {\n    Active,\n    Inactive,\n    Pending,\n}\n\nlet current = Status.Active;\nlet msg = match current {\n    Status.Active => "Online",\n    Status.Inactive => "Offline",\n    Status.Pending => "Transitioning",\n};\nterm.println("Status: " + msg);`
    },
    {
        name: "Error Handling",
        description: "Standard error propagation with the ? operator.",
        code: `enum Result {\n    Ok(num),\n    Err(str),\n}\n\nfn divide(a: num, b: num) -> Result {\n    if (b == 0) { return Result.Err("Division by zero"); }\n    return Result.Ok(a / b);\n}\n\nlet success = divide(10, 2)?;\nterm.println("Result: " + success);\n\n// This will propagate error if it fails\n// let failure = divide(10, 0)?;\n// term.println(failure);`
    }
  ];
  ...
  useEffect(() => {
    init().then(() => {
      ...
      setOutput("Error loading runtime environment.");
      setIsLoading(false);
    });
  }, []);
  ...
  const handleFormat = () => {
    if (!isWasmLoaded) return;
    try {
      if (typeof format_code === 'function') {
        const result = format_code(code);
        setCode(result);
      } else {
        console.warn("Formatting not available in current WASM build");
      }
    } catch (e) {
      setOutput(`Format error: ${e}`);
    }
  };
  ...
  return (
    <Layout fullWidth>
      <div className="flex flex-col md:flex-row min-h-[calc(100vh-64px)] bg-bio-cream overflow-hidden">
        ...
        {/* Sidebar */}
        <aside className="w-full md:w-64 bg-bio-offwhite border-r border-bio-black/10 p-6 md:h-[calc(100vh-64px)] md:sticky md:top-16 overflow-y-auto shrink-0 shadow-inner">
          ...
          <div className="space-y-3">
            {examples.map((example) => (
              ...
              >
                <div className="font-bold text-bio-black group-hover:text-bio-green transition-colors">{example.name}</div>
